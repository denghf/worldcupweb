...
> > 最后更新：2026-06-15
> 版本：v2.4 【本次更新】管理员后台下注详情新增右侧赛事下注概览，用于按赛事核对赔率选项和下注明细联动
...
│   │   ├── 结算结果（中奖/未中奖/赔付金额）
│   │   ├── 右侧赛事下注概览（仅统计当前筛选结果）
│   │   ├── 每场赛事分为赛事详情 / 赔率详情 / 下注详情三个区域
│   │   ├── 点击赔率选项，高亮该场下注详情中的对应下注记录
│   │   └── 右侧区域独立展示，不与左侧下注列表联动
│   ├── 🔴 3. 玩家管理
│   │   ├── 玩家列表（昵称/余额/下注数/盈亏）
│   │   ├── 添加新玩家
...
│          │  下注详情                           │  赛事下注概览              │
│  1.下注  ├────────────────────────────────────┼────────────────────────────┤
│  管理    │  筛选: [全部] [已下注] [已中奖]      │  当前筛选                  │
│          │        [玩家▼]                      │  红色=当前筛选中有人下注   │
│  2.下注  │                                    │                            │
│  详情 ← │  ┌──────────────────────────────┐  │  ┌──────────────────────┐  │
│          │  │ 小明  单关  已下注            │  │  │ 赛事详情              │  │
│  3.玩家  │  │ 巴西胜 @2.10  下注200         │  │  │ 巴西 vs 德国          │  │
│  管理    │  └──────────────────────────────┘  │  │ 06/15 22:00           │  │
│          │                                    │  ├──────────────────────┤  │
│  4.赛事  │  ┌──────────────────────────────┐  │  │ 赔率详情              │  │
│  管理    │  │ 老王  串关×3  已下注          │  │  │ 胜平负: [胜红][平][负]│  │
│          │  │ 巴西胜 + 阿根廷平 ...         │  │  │ 总进球: [0][1][2红]   │  │
│  5.赛果  │  └──────────────────────────────┘  │  ├──────────────────────┤  │
│  查看    │                                    │  │ 下注详情 共 2 条       │  │
│          │  右侧区域独立展示：                │  │ ▶ 小明 胜平负 胜 @2.10│  │
│  6.兑奖  │  - 不展开左侧注单                  │  │   老王 总进球 2球 @3.2│  │
│  管理    │  - 不滚动左侧列表                  │  │                        │  │
│          │  - 只做右侧内部高亮                │  │ 点击赔率“胜”：         │  │
│  7.统计  │                                    │  │ 下方对应下注记录高亮   │  │
│          │                                    │  └──────────────────────┘  │
...
| **【本次更新】下注详情赛事概览** | 按赛事查看当前筛选下的赔率选项和下注明细 | 点击赔率只高亮右侧对应下注，不联动左侧列表 |
...
**【本次更新】下注详情右侧赛事下注概览交互细节**：

| 场景 | 交互处理 |
|------|---------|
| 进入下注详情页 | 右侧“赛事下注概览”按当前状态筛选、玩家筛选后的下注数据生成，不展示筛选结果外的数据 |
| 按赛事分组 | 每场赛事独立成卡片，卡片固定分为“赛事详情 / 赔率详情 / 下注详情”三个区域 |
| 赛事详情 | 展示主队、客队、开赛时间；如已有比分，可展示全场比分 |
| 赔率详情 | 展示该赛事各玩法赔率；当前筛选结果中有人下注过的赔率选项标红，未下注选项保持普通状态 |
| 下注详情 | 默认展示该场赛事在当前筛选条件下的全部下注记录，并显示总条数；单条记录展示格式为“用户 - 下注内容 - 赔率” |
| 点击赔率选项 | 高亮下注详情中与该赔率选项匹配的下注记录，匹配维度为 matchId + betMarket + selectedOption |
| 点击下注记录 | 右侧下注记录仅用于查看，不触发展开、滚动或高亮左侧下注列表 |
| 切换筛选条件 | 左侧下注列表和右侧赛事下注概览同步刷新，清空当前赔率选中态和右侧下注高亮态 |
| 空状态 | 当前筛选下无下注时，右侧显示“当前筛选下暂无下注赛事” |

**【本次更新】下注详情数据规范**：

| 字段名 | 数据类型 | 是否必填 | 说明 |
|------|--------|--------|------|
| matchId | number | 是 | 赛事分组唯一标识 |
| homeTeam / awayTeam | string | 是 | 赛事详情展示用 |
| kickoffTime | datetime | 是 | 赛事详情展示用 |
| odds[] | array | 是 | 赔率详情展示用，包含 betType、optionKey、oddsValue |
| betId | number | 是 | 左侧注单展开定位用 |
| betUid | string | 是 | 下注详情展示用 |
| itemId | number | 是 | 左侧具体下注项定位和高亮用 |
| betMarket | enum | 是 | 玩法类型，用于赔率和下注记录匹配 |
| selectedOption | string | 是 | 下注选项，用于赔率和下注记录匹配 |
| userNickname | string | 是 | 下注详情中的“用户” |
| betContent | string | 是 | 下注详情中的“下注内容”，由 betMarket + selectedOption 格式化得到 |
| lockedOdds | decimal | 是 | 下注详情中的“赔率”，展示为 @ odds |
| totalAmount | decimal | 是 | 下注金额，可在左侧详情或后续扩展中展示 |
| result | enum | 是 | pending / won / lost / cancelled |
...
| 串关摘要 | 串关 {N}场，综合赔率 {赔率} |
| 【本次更新】下注详情概览标题 | 赛事下注概览 |
| 【本次更新】下注概览空状态 | 当前筛选下暂无下注赛事 |
| 【本次更新】下注概览提示 | 红色表示当前筛选结果中有人下注 |
| 【本次更新】赛果查看标题 | 赛果查看 |
...
- 【本次更新】管理员后台“下注详情”右侧赛事下注概览：按当前筛选展示赛事详情、赔率详情、下注详情，并支持赔率到下注记录高亮联动
